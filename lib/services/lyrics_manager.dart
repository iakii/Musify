...
class LyricsManager {
  Future<String?> fetchLyrics(String artistName, String title) async {
    ...
    final lyricsFromLyricsApi = await _fetchLyricsFromApi(artistName, title);

    return lyricsFromLyricsApi;
  }

  Future<String?> _fetchLyricsFromApi(
    String artistName,
    String title,
  ) async {
    final url =
        'https://api.lrc.cx/api/v1/lyrics/single?title=$title&album=海阔天空&artist=$artistName';
